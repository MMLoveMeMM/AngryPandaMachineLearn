# -*- coding: utf-8 -*-
"""
Created on Mon Nov 20 17:44:24 2017

@author: rd0348
演示tensorboard 如何使用
一定要添加训练数据,前面很多都有没有测试数据,结果导致没有效果生成
TensorFlow API 1.2
步骤 : 
<1> : 先创建图;
<2> : 训练数据;
<3> : 启动anaconda cmd,切换到数据路径下,执行 : tensorboard --logdir mnist_tensorboard
<4> : 然后打开chrome 浏览器输入 : http://127.0.0.1:6006
"""

import tensorflow as tf    
import numpy as np    

LOCAL_TENSORFLOW_PATH = "mnist_tensorboard\"
...
#选定可视化存储目录    
writer = tf.summary.FileWriter(LOCAL_TENSORFLOW_PATH,sess.graph)    
